...
class UsingProbesSpec extends ScalaTestWithActorTestKit with AnyWordSpecLike {
  ...
  "A master actor" should {
    ...
    "send a task to the worker actor" in {
      ...
      workerProbe.expectMessage(WorkerTask(taskString, master, externalProbe.ref))
...
object UsingProbesSpec {
  ...
  case class WorkCompleted(result: Int, originalDestination: ActorRef[ExternalProtocol]) extends MasterProtocol
  case class Register(workerRef: ActorRef[WorkerTask], replyTo: ActorRef[ExternalProtocol]) extends MasterProtocol

  case class WorkerTask(text: String, master: ActorRef[MasterProtocol], originalDestination: ActorRef[ExternalProtocol]) extends MasterProtocol
      ...
      Behaviors.receive { (context, message) =>
        message match {
          case Work(text, replyTo) =>
            ...
            Behaviors.same
          case WorkCompleted(result, originalDestination) =>
            val newTotal = totalCount + result
            originalDestination ! Report(newTotal)
            active(workerRef, newTotal)
        }
    }
  }

  object Requester {
    def apply(): Behavior[ExternalProtocol] = Behaviors.receive { (context, message) =>
      message match {
        case RegisterAck =>
          Behaviors.same
        case Report(totalCount) =>
          context.log.info(s"Report: totalCount is ${totalCount}")
          Behaviors.same
      }
    }
  }
